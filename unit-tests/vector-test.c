...
const char* tst_vec_push_null(void) {
    ...
    ret = make_vector(&vec, sizeof(int), 0);
    if (ret == -1)
        return INIT_FAIL;
...
const char* tst_vec_reserve(void) {
    ...
    ret = make_vector(&vec, sizeof(int), 200);
    if (ret != 0)
        return INIT_FAIL;
    ...
    for (size_t i = 10; i < 550; i += 10) {
        ...
        for (size_t j = 0; j < i-10; j++) {
            ...
            if ((size_t)*(int *)vec_ref(&vec, j) != j) {
                ...
                snprintf(msg_buff, 50, "memory corrupted at j=%zu; i=%zu: %d",
...
const char* tst_vec_resize(void) {
    ...
    if ((ret == 0 && vec.len != SIZE_MAX) ||
        (ret == 0 && vec.data == NULL)) {
        error_message = "failed to resize properly (SIZE_MAX)";
        goto cleanup_return;
    }

    if (ret == 0 && vec.capacity < SIZE_MAX) {
        error_message = "failed to allocate enough space.";
        goto cleanup_return;
    }
...
const char* tst_vec_set_out_of_bounds(void) {
    ...
    ret = make_vector(&vec, sizeof(int), 10);
    if (ret != 0)
        return INIT_FAIL;
